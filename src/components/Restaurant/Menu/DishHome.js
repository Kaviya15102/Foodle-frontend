...
import RestaurantSearch from "../../UserSide/HomePage/RestaurantSearch";
...
const DishHome = () => {
  ...
  const deleteDishes=async (id)=>{
    console.log(id);
    await axios.delete(`http://localhost:8090/menu-item?menuId=${id}`)
      .then((res) => { 
  ...
  useEffect(() => { 
    console.log(id);
    axios.get("http://localhost:8090/restaurant/all")
      .then((response) => { 
